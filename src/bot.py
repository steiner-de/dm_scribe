...
def main():
    ...
    @bot.tree.command(name='name_me', description="Set the bot's nickname in this server (used in transcriptions and join messages).")
    async def name_me(interaction: discord.Interaction, name: str):
        try:
            ...
            await interaction.response.send_message("I don't have permission to change my nickname here.")
        except Exception as e:
    ...
    async def leave_voice(interaction: discord.Interaction):
        voice_client = bot.current_voice_client or bot.voice_handler.get_voice_client(interaction.guild.id)
    ...
    async def play(interaction: discord.Interaction):
        ...
        if voice_client is None or not voice_client.is_connected():
            await interaction.response.send_message("I need to join a voice channel first. Use /join.")
            return
    ...
    async def stop(interaction: discord.Interaction):
        ...
        if filename:
            ...
            if not is_ollama_running():
                await interaction.followup.send("Ollama server not running. Starting it now (this may take a moment)...")
                if await start_ollama_server():
                    await interaction.followup.send("Ollama server started successfully!")
                else:
                    await interaction.followup.send("Failed to start Ollama server. Ensure Ollama is installed and try again.")
                    os.remove(filename)
                    return
            ...
            enhanced_transcription = bot.transcriber.enhance_transcription(transcription, bot.character_map)
            ...
            if guild_id in bot.notes_channel_map:
                ...
                if notes_channel:
                    ...
                    try:
                        ...
                        await interaction.followup.send(f"⚠️ Cannot post to {notes_channel.mention} (permission denied)")

            await interaction.followup.send(f"**Transcription:** {enhanced_transcription}\n\n**Summary:** {summary}")
            os.remove(filename)
        else:
            await interaction.followup.send("Not currently recording.")

    @bot.tree.command(name='assign_dm', description='Assign a Discord user as the DM for better transcription context.')
    async def assign_dm(interaction: discord.Interaction, user: discord.User):
        bot.character_map[str(user.id)] = {"name": "DM", "class": "Dungeon Master", "species": "N/A"}
        bot.save_character_map()
        await interaction.response.send_message(f"Assigned {user.mention} as DM.")

    @bot.tree.command(name='assign_character', description='Assign a character name to a Discord user for transcription enhancement.')
    ...
    @bot.tree.command(name='remove_character', description='Remove a user\'s assigned character name.')
    async def remove_character(interaction: discord.Interaction, user: discord.User):
        if str(user.id) in bot.character_map:
            ...
            bot.save_character_map()
            await interaction.response.send_message(f"Removed character assignment for {user.mention}.")
        else:
    ...
    async def sync_commands(interaction: discord.Interaction):
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("You need administrator permissions to sync commands.")
            return
        await interaction.response.defer()
        try:
    ...
    async def list_characters(interaction: discord.Interaction):
        if bot.character_map:
            ...
            for handle, info in bot.character_map.items():
                ...
                char_list.append(f"{display_name}: {info['name']} ({info.get('class', 'N/A')}, {info.get('species', 'N/A')}, {info.get('gender', 'N/A')})")
            await interaction.response.send_message(f"Assigned characters:\n{chr(10).join(char_list)}")
        else:
            await interaction.response.send_message("No characters assigned yet.")

    @bot.tree.command(name='set_notes_channel', description='Set the Discord channel where session notes will be posted (admin only).')
    async def set_notes_channel(interaction: discord.Interaction, channel: discord.TextChannel):
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("You need administrator permissions to set the notes channel.")
            return
        guild_id = str(interaction.guild.id)
    ...
    @bot.tree.command(name='get_notes_channel', description='Get the current notes channel for this server.')
    async def get_notes_channel(interaction: discord.Interaction):
        ...
        if guild_id in bot.notes_channel_map:
            ...
            if channel:
                ...
                await interaction.response.send_message("Notes channel is set but no longer exists.")
        else:
            await interaction.response.send_message("No notes channel set for this server. Use `/set_notes_channel #channel` to set one.")
    ...
    async def help_command(interaction: discord.Interaction):
        help_message = (
            ...
            "/assign_character [user] [character_name] [class] [species] [gender] - Assign a character name\n"
