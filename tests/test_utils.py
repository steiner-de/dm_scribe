...
def test_save_transcript_for_training_creates_file(tmp_path, monkeypatch):
    ...
    utils.save_transcript_for_training("Hello world", "session1", "general")
...
def test_export_training_data_combines_files(tmp_path, monkeypatch):
    repo_root = tmp_path / "repo"
    training_data = repo_root / "training_data"
    ...
    assert len(lines) == 1
    assert json.loads(lines[0])["session_id"] == "session1"
